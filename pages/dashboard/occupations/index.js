...
export default function DashboardOccupations() {
    ...
    const handleDelete = async (e, id) => {
        if (confirm('Hapus pekerjaan ini?')) {
            const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/o/${id}/delete?token=${window.localStorage.getItem('t')}`)
        ...
        <div className='body'>
            ...
            <div className='main-container'>
                <main>
                    ...
                    { list.length == 0 ? <p>Data tidak ditemukan</p> : <table className='table'><thead><tr><td>Id</td><td>Nama</td><td>Order</td><td></td><td></td></tr></thead><tbody>{ list.map((item) => <tr key={item.id}><td>{item.id}</td><td>{item.name}</td><td>{item.order}</td><td><Link href={`/dashboard/occupations/${item.id}/edit`}><img className='iconDel' src='/images/icon-edit.png' alt='edit' /></Link></td><td><img className='iconDel' src='/images/icon-del.png' alt='delete' onClick={(event) => handleDelete(event, item.id)}/></td></tr>) }</tbody></table> }

                    <Link className={style.btn} href='/dashboard/occupations/create'>
                        <button className={`btn blue-btn ${style.btnCreate}`}>Tambah Pekerjaan</button>
                    </Link>
                </main>
            </div>
