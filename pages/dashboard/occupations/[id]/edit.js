...
export default function DashboardEditOccupation() {
    ...
    useEffect(() => {
        ...
        if (t == null) {
            ...
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/o/${router.query.id}`)
                .then((res) => res.json())
                .then((val) => {
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/o/${router.query.id}/edit?token=${window.localStorage.getItem('t')}`, {
                name,
                order
            })
