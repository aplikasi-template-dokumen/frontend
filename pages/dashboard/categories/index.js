            ...
            <div className='main-container'>
                <main>
                    ...
                    <div className={display[0]}>
                        ...
                        {/* { catList.length == 0 ? <p>Data tidak ditemukan</p> : <table className='table'><thead><tr><td>Id</td><td>Nama Kategori</td><td></td></tr></thead><tbody>{ catList.map((item) => <tr key={item.id}><td>{item.id}</td><td>{item.name}</td><td><Link href={`/dashboard/categories/${item.id}/edit`}><img className='iconDel' src='/images/icon-edit.png' alt='edit' /></Link><img className='iconDel' src='/images/icon-del.png' alt='delete' onClick={((event) => handleDelete(event, item.id, 'c'))} /></td></tr>) }</tbody></table> } */}
